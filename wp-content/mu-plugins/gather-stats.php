...
WP_CLI::add_command(
	...
	function ( $args ) {
		...
		chdir( ABSPATH . '/results/theme-directories/wp-themes' );
...
function print_table( $rows ) {
	...
	$lines = [];
	$lines[] = "Theme | Original CSS | Minified CSS | Budget % | Status";
	$lines[] = ":---- | -----------: | -----------: | -------: | :----:";
	foreach ( $rows as $row ) {
		$output = [];
		// $output[] = isset( $popularities[ $row[0] ] ) ? $popularities[ $row[0] ] + 1 : '?';
		$output[] = sprintf( '[%1$s](https://wordpress.com/themes/%1$s/)', $row[0] );
